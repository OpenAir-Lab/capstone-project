...
#include "core/system_state.h"      //used in keeping the state updated
#include "ui/ui_display.h"
#include "inputs/input_hmi.h"
#include "tasks/task_system.h"
#include "tasks/task_display.h"
#include "tasks/task_rf.h"
...
SystemState_t state;              // Gives every task access to one shared state
...
void setup(void) {
    ...
    // =======================
    state.rx_freq_hz =   446000000;   // Default Rx frequency
    state.is_tx = false;
    state.ptt_pressed = false;
    state.rssi_dbm = -120;          // CC1200 noise floor until updated
    state.battery_percent = 100;    // Temporary placeholder until BMS is read

    // ----------------------------
    // Tasks
    // ----------------------------
    xTaskCreatePinnedToCore(systemTask,  "System",  4096, NULL, 2, NULL, 1);
    xTaskCreatePinnedToCore(displayTask, "Display", 4096, NULL, 1, NULL, 1);
    xTaskCreatePinnedToCore(rfTask,      "RF",      4096, NULL, 3, NULL, 1);
    //All of this will need to be verified and tested for validation
    /* Once this works:
    Add audio (microphone & speaker)
    Add menus
    Add band switching
    Add data structures for channel memories
    Add multi-modulino coordination
...
void loop() {
    debug_print_state(state);   //test state updates 
    delay(10000);
